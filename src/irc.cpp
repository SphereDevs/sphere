...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #spherecoinTEST\r");
            Send(hSocket, "WHO #spherecoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #spherecoin\n");
            // randomly join #spherecoin00-#spherecoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #spherecoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #spherecoin%02d\r", channel_number).c_str());
        };
