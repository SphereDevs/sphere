...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSphereAmount"))
        settings.setValue("nAnonymizeSphereAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeSphereAmount = settings.value("nAnonymizeSphereAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeSphereAmount"))
        SoftSetArg("-anonymizesphereamount", settings.value("nAnonymizeSphereAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeSphereAmount:
            nAnonymizeSphereAmount = value.toInt();
            settings.setValue("nAnonymizeSphereAmount", nAnonymizeSphereAmount);
            emit anonymizeSphereAmountChanged(nAnonymizeSphereAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
